...
export const handleAchievementProgress = (name, saveGame, setUnlocked) => {
  ...
  case 'Craft Master':
    ...
    progress.valueOne = counter;
    progress.valueTwo = craftingRecipes.length;
    progress.valueThree = true;
    break;
  case 'Fisherman': 
  case 'Ol\' Mariner': 
  case 'Master Angler':
  ...
  case 'Mother Catch':
    counter = 0;
    const fish = saveGame.player[0].fishCaught[0].item;
  ...
  case 'Polyculture':
    ...
    if (itemsShipped) {      
      itemsShipped.forEach(item => {
        ...
        if (crops.hasOwnProperty(itemCode) && amountShipped < 15) {
          counter++;
        }
      });
    }
  ...
  case 'Full Shipment':
    ...
    if (itemsShipped) {      
      itemsShipped.forEach(item => {
        const itemCode = item.key[0].int[0];
        if (fullShipmentCodes.includes(Number(itemCode))) {
          counter++;
        }
      });
    }
  ...
  const achievementProgress = {
    ...
    'A New Friend': {progressDetail: `Closest friend is ${progress.valueOne}. ${(1250) - progress.valueTwo} friendship points away.`},
    'Best Friends': {progressDetail: `Closest friend is ${progress.valueOne}. ${(2500) - progress.valueTwo} friendship points away.`},
    'The Beloved Farmer': {progressDetail: `${8 - progress.valueOne} friends remaining.`},
    'Cliques': {progressDetail: `${4 - progress.valueOne} friends remaining.`},
    'Networking': {progressDetail: `${10 - progress.valueOne} friends remaining.`},
    'Popular': {progressDetail: `${20 - progress.valueOne} friends remaining.`},
    'Cook': {progressDetail: `${10 - progress.valueOne} recipes remaining.`},
    'Sous Chef': {progressDetail: `${25 - progress.valueOne} recipes remaining.`},
    'Gourmet Chef': {progressDetail: `${80 - progress.valueOne} recipes remaining.`},
    'Moving Up': {progressDetail: 'Requires 10,000g and 450 Wood'},
    'Living Large': {progressDetail: 'Requires 50,000g and 150 Hardwood'},
    'D.I.Y': {progressDetail: `${15 - progress.valueOne} items remaining.`},
    'Artisan': {progressDetail: `${30 - progress.valueOne} items remaining.`},
    'Craft Master': {progressDetail: 
      `${129 - progress.valueOne} items remaining. 
      ${progress.valueThree ? `${129 - progress.valueTwo} recipes not learnt.` : ''}`},
    'Fisherman': {progressDetail: `${10 - progress.valueOne} different fish remaining.`},
    'Ol\' Mariner': {progressDetail: `${24 - progress.valueOne} different fish remaining.`},
    'Master Angler': {progressDetail: `${67 - progress.valueOne} different fish remaining.`},
    'Mother Catch': {progressDetail: `${100 - progress.valueOne} fish remaining.`},
    'Treasure Trove': {progressDetail: `${formatNumber(40 - progress.valueOne)} items remaining.`},
    'Gofer': {progressDetail: `${formatNumber(10 - progress.valueOne)} quests remaining.`},
    'A Big Help': {progressDetail: `${formatNumber(40 - progress.valueOne)} quests remaining.`},
    'Polyculture': {progressDetail: `${formatNumber(28 - progress.valueOne)} crops remaining.`},
    ...
    'Full Shipment': {progressDetail: `${formatNumber(145 - progress.valueOne)} items remaining.`},
    ...
    'The Bottom': {progressDetail: `${progress.valueOne === undefined ? '' : `${120 - progress.valueTwo} floors remaining.`}`},
    'Local Legend': {progressDetail: 
      `${progress.valueOne === undefined ? 'Joja warehouse developed.' : progress.valueOne ? '' : `${30 - progress.valueTwo} bundles remaining.`}`},
    ...
    'Master Of The Five Ways': {progressDetail: `${progress.valueOne ? '' : `${5 - progress.valueTwo} skills remaining.`}`},
